...
| Значение | Описание | Кто может выдавать |
|---|---|---|
| `user` | Конкретный пользователь по `subjectId` (UUID) | Любой аутентифицированный |
| `group` | Группа пользователей по `subjectId` (UUID) | Любой аутентифицированный |
| `everyone` | Все аутентифицированные пользователи | Только системный admin (`UserRole.ADMIN`) |

Для `subjectType = everyone` поле `subjectId` не требуется. Выданные права применяются ко всем пользователям при проверке `checkForUser()`.
